...
vi A;
V<vi> adj;
V<AR<bool, 32>> dp, stor;

AR<bool, 32> operator+(AR<bool, 32> a, AR<bool, 32> b) {
	AR<bool, 32> res{};
	F0R(i, 32) F0R(j, 32) res[i ^ j] |= a[i] & b[j];
	return res;
}
...
void solve(int x, int p) {
	...
	if (sub[x] % 2 == 0) { dp[x][0] = 1; }
}

vi ans;

void reconstruct(int v, int x, int p) {
	assert(dp[x][v]);
	if (sub[x] % 2 == 0 && v == 0) {
		ans.pb(x);
		return;
