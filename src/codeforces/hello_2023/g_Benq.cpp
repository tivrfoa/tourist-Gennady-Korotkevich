...
void solve(int tc) {
	...
	auto go = [&]() {
		int ans = N;
		int x = N - 1;
		while (x >= 0 && roads[2].at(x) == '0') --x;
		int y = N - 1;
		while (y >= 0 && roads[0].at(y) == '1') --y;
		int z = N - 1;
		while (z >= 0 && roads[1].at(z) == '1') --z;
		ckmin(ans, 2 * (N - 1 - x));
		ckmin(ans, N - 1 - x + N - 1 - y);
		ckmin(ans, N - 1 - z + N - 1 - x);
		ckmin(ans, N - 1 - z + N - 1 - y);
		ps(ans);
	};
	while (true) {
		...
		each(t, roads) { each(u, t) u = '0' + '1' - u; }
		if (roads[0].bk == '1' && roads[1].bk == '1' && roads[2].bk == '0') {
			return go();
		}
		rotate(begin(roads), 1 + all(roads));
	}
}
